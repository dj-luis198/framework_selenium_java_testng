package com.demoqa.utils.web_interactions;

//Métodos para interactuar con formularios:
//fillForm(WebElement form, Map<String, String> data)
//submitForm(WebElement form)
//resetForm(WebElement form)
//getFormValues(WebElement form)
//setFormValue(WebElement form, String fieldName, String value)
//getFormFields(WebElement form)
//getFormFieldValue(WebElement form, String fieldName)

public class FormActions {
    public void metodo() {

    }
}
